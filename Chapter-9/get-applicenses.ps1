...
$url = "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps?`$filter=isof('microsoft.graph.macOsVppApp') or isof('microsoft.graph.iosVppApp') or isof('microsoft.graph.microsoftStoreForBusinessApp') or isof('microsoft.graph.androidManagedStoreApp') or isof('microsoft.graph.androidManagedStoreWebApp')"

$allapps = getallpagination -url $url

$applist = $allapps | Select-Object DisplayName, totalLicenseCount, usedLicenseCount, @{Name="availableLicenseCount";Expression={$_.totalLicenseCount - $_.usedLicenseCount}}
